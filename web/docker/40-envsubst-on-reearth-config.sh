...
# Rewrite favicon in index.html only if REEARTH_FAVICON_URL is set
if [ -n "$REEARTH_FAVICON_URL" ]; then
  sed -i -e "s|<link rel=\"icon\" href=\"[^\"]*\" type=\"image/x-icon\" />|<link rel=\"icon\" href=\"${REEARTH_FAVICON_URL}\" type=\"image/x-icon\" />|g" "$_REEARTH_HTML_FILE"
fi
...
wrap_reearth_variables() {
    for var in $(env | grep "^$_REEARTH_PREFIX" | cut -d= -f1); do
        value=$(printenv "$var")
        if [ -z "$value" ]; then
            eval "export $var='\"\"'"
