...
func InitializeEcho(ctx context.Context, dev bool) (*echo.Server, error) {
	...
	findByIDsUsecase := assetuc.NewFindByIDsUsecase(mongoAsset, project, workspace, policy)
	findByProjectUsecase := assetuc.NewFindByProjectUsecase(mongoAsset, project, workspace, policy)
	storage := gcp.NewStorage()
	createUsecase := assetuc.NewCreateUsecase(mongoAsset, project, workspace, policy, storage)
	updateUsecase := assetuc.NewUpdateUsecase(mongoAsset, project, workspace, policy)
	...
	findBySubUsecase := useruc.NewFindBySubUsecase(user)
